...
def main():
    ...
    plt.axis((0, 52 * years, 100, 0))
    plt.xlabel('Years')
    plt.ylabel('Percent Reduction from Baseline')

    matedPairs = plt.plot(t, matedPairsRed, 'm--', label= 'Infection Intensity', linewidth= 4)
    prevalence = plt.plot(t, prevalenceRed, 'b-.', label= 'Prevalence', linewidth= 4)
    reproNumber = plt.plot(t, reproNumberRed, 'k', label= 'Reproductive No.', linewidth= 4)
    currentAxis = plt.gca()
    plt.legend()

    fig.savefig("GeneDriveReviewFigure1a.png")
    fig.savefig("GeneDriveReviewFigure1a.tiff")
